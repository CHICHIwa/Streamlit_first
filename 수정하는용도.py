...
with tab1:
  ...
  st.header('프로젝트의 목표')
  # 마크다운 문법 지원
  st.markdown('1. 서울시의 전세 가격을 예측하는 머신러닝 모델을 개발하고, 이를 **실제 문제 해결**에 적용한다.')
  st.markdown('2. 두명이서 프로젝트를 진행하기에 모든 과정을 함께 진행하며 **최대한 다양하게 파이썬을 이용**해본다.')
  st.markdown('3. 활용은 **재미있게** 한다')
  # 선 그리기
  st.markdown('---')
...
with tab5:
  ...
  st.image(img)
  st.subheader('▷ MSE: 8052.51 / :blue[RMSE: 13611.29] / :red[R2 SCORE: 0.75]')
...
with tab6:
  ...
  st.write('>Grid Search로 최적의 방법을 찾고자 했지만, 노트북 성능 이슈 (한계점에 넣기)')
...
with tab7:
  ...
  st.subheader('추가한 외부데이터 :red[cctv_수량/명소]가 모델에서 중요하게 작용')
...
with tab11:
  ...
  st.write('> 데이터의 양이 늘어나자 :blue[노트북 성능의 한계]로 모델링과 Greid Search활용이 불가능해지며 한계 봉착')
